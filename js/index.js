...
async function show_atlas() {
	...
	if (show_nebulas) {
		...
		nebulas.forEach(function(neb) {
			[neb_xf, neb_yf] = remap(neb.x, neb.y);
			drawArc(neb_xf, neb_yf, neb.r * scale_size * a / galaxy_radius, "rgba(63,63,63,0.5)", [8 * scale_size_f, 3 * scale_size_f], scale_size_f);
		});
	}
...
function mark_habit() {
	galactic_objects.forEach(function (star) {
		for(var i = 0; i < star.habit_num; i ++) {
			drawCircle(star.xf + scale_size_f * (4 + 3 * i), star.yf - scale_size_f * 4.5, scale_size_f * 2, "rgba(0," + (170 - 34 * i) + "," + (17 * i) + ",0.65)");
		}
	});
}
...
function load() {
	...
	if (!loaded) {
		load_constructions_queues();
		load_starbases(); // must before load_galactic_objects()
		load_planets();
		load_galactic_objects();
		border_px = 2 + Math.ceil(galactic_objects.size / 211);
		load_nebulas();
		load_wormholes();
		load_country();
	}
...
/* 加载建造队列 */
function load_constructions_queues() {
	var cqueues_start = gamestate.indexOf("\nconstruction") + 41;
	var cqueues_end = gamestate.indexOf("\n\titem");
	var cqueues_str = gamestate.substring(cqueues_start, cqueues_end).replaceAll(/\t\d+=none\n/g, "");
	cqueues = new Map();
	for (var i = 0; i < 100000 && cqueues_str.length > 10; i++) {
		var end = cqueues_str.substring(2).search(/\n\t\t\t\d+=/g);
		end = end == -1 ? cqueues_str.length - 1 : end + 2;
		queue_current = cqueues_str.substring(0, end);
		var id = parseInt(queue_current.substring(2, queue_current.indexOf("=")));
		var owner = parseInt((/\n\t+owner=\d+/g.exec(queue_current) || [""])[0].replaceAll(/\n\t+owner=/g, ""));
		cqueues_str = cqueues_str.substring(end);
		if (owner != 4294967295) {
			cqueues.set(id, owner)
		}
	}
}
...
function load_starbases() {
	...
	for (var i = 0; i < 0x10000 && starbases_str.length > 10; i++) {
		...
		var owner = cqueues.get(parseInt(/build_queue=\d+/g.exec(base_str)[0].replace("build_queue=", "")));
...
function load_nebulas() {
	nebulas = [];
	var nebulas_strs = Array.from(gamestate.match(/nebula={[\s\S]+?\n}/g));
	nebulas_strs.forEach(function (str) {
		nebulas.push(new nebula(str));
	});
}
...
function load_wormholes() {
	...
	for (var i = 0; i < 0x1000 && natural_wormholes_str.length > 10; i++) {
		...
		if (bypasses.get(bp_id) != undefined) {
...
function galactic_object(str) {
	...
	this.name = (/\n\t+name={\n\t+key=.+/g.exec(str) || [""])[0].replaceAll(/\n\t+name={\n\t+key=/g, "").replaceAll(/[\"\n]/g, "");
	this.star_class = /\n\t\tstar_class=.+\n/g.exec(str)[0].replace("\n\t\tstar_class=", "").replaceAll(/[\"\n]/g, "");
	this.initializer = /\n\t\tinitializer=.+\n/g.exec(str)[0].replace("\n\t\tinitializer=", "").replaceAll(/[\"\n]/g, "");
	this.planets = Array.from(str.match(/\n\t+planet.+/g), m => parseInt(m.replaceAll(/\n\t+planet=/g, "").replaceAll(/[\"\n]/g, "")));
	this.habit_planets = [];
	this.hyperlanes = [];
	this.owner = -1;
	this.starbase = -1;
	...
	function init() {
		...
		if (str.indexOf("starbase=") != 0) {
			this.starbase = parseInt((/\n\t+starbases={\n\t+\d+/g.exec(str)[0] || [""]).replaceAll(/\n\t+starbases={\n\t+/g, ""));
			this.owner = starbases.get(this.starbase);
		}
		for (var j = 0; j < this.planets.length; j++) {
			var p = planets.get(this.planets[j]);
			var hab_index = habitable.indexOf(p.planet_class);
			if(hab_index != -1) {
				this.habit_planets.push([habitable_name[hab_index], p.planet_size]);
			}
		}
		this.habit_num = this.habit_planets.length;
	}
}
/* --国家 */
function empire(str) {
	...
	this.economy_power = parseInt((/\n\t\teconomy_power=\d+/g.exec(str)[0] || [""]).replace("\n\t\teconomy_power=", ""));
	this.tech_power = parseInt((/\n\t\ttech_power=\d+/g.exec(str)[0] || [""]).replace("\n\t\ttech_power=", ""));
...
function planet(str) {
	this.name = (/\n\t+name={\n\t+key=.+/g.exec(str) || [""])[0].replaceAll(/\n\t+name={\n\t+key=/g, "").replaceAll(/[\"\n]/g, "");
	this.planet_size = (/\n\t+planet_size=.+\n/g.exec(str) || [""])[0].replace("\n\t\t\tplanet_size=", "").replaceAll(/[\"\n]/g, "");
	this.planet_class = (/\n\t+planet_class=.+\n/g.exec(str) || [""])[0].replace("\n\t\t\tplanet_class=", "").replaceAll(/[\"\n]/g, "");
...
function nebula(str) {
	...
	this.x = parseFloat((/\n\t\tx=[-.\d]{1,10}/g.exec(str) || [""])[0].replace("\n\t\tx=", ""));
	this.y = parseFloat((/\n\t\ty=[-.\d]{1,10}/g.exec(str) || [""])[0].replace("\n\t\ty=", ""));
	this.r = parseFloat((/\n\tradius=[-.\d]{1,10}/g.exec(str) || [""])[0].replace("\n\tradius=", ""));
}
...
function add_hyperlane(id1, id2) {
	...
	var dis = Math.round(Math.sqrt(Math.pow(star1.x - star2.x, 2) + Math.pow(star1.y - star2.y, 2)));
	...
	switch (cmds[1]) {
		...
		case "country":
			...
			if (country.military_power < 1000) {
				output += "舰队实力: " + country.military_power;
			} else if (country.military_power < 1000000) {
				output += "舰队实力: " + (country.military_power / 1000).toFixed(1) + 'k';
			} else {
				output += "舰队实力: " + (country.military_power / 1000000).toFixed(1) + 'm';
			}
			output += "\n";
			if (country.economy_power < 1000) {
				output += "经济实力: " + country.economy_power;
			} else if (country.economy_power < 1000000) {
				output += "经济实力: " + (country.economy_power / 1000).toFixed(1) + 'k';
			} else {
				output += "经济实力: " + (country.economy_power / 1000000).toFixed(1) + 'm';
			}
			output += "\n";
			if (country.tech_power < 1000) {
				output += "科技水平: " + country.tech_power;
			} else if (country.tech_power < 1000000) {
				output += "科技水平: " + (country.tech_power / 1000).toFixed(1) + 'k';
			} else {
				output += "科技水平: " + (country.tech_power / 1000000).toFixed(1) + 'm';
			}
			output += "\n";
			return output;
		case "star":
			...
			if (star != null) {
				...
				if(star.habit_num > 0) {
					output += "宜居星球(类型:规模): \n";
					star.habit_planets.forEach(function(value) {
...
canvas.onmousewheel = function (e) {
	...
	sf = delta > 0 ? 1.1 : 0.9;
	scale_size *= sf;
	scale_center[0] = scale_center[0] * sf - w / 2 * (sf - 1) + (w / 2 - move_event.clientX) * (sf - 1);
	scale_center[1] = scale_center[1] * sf - h / 2 * (sf - 1) + (h / 2 - move_event.clientY) * (sf - 1);
	if (gamestate.length != 0) {
		show_atlas();
	}
}
